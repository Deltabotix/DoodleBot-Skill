...
// ── State ────────────────────────────────────────────────────────────
let timeline = [];   // flat list of ops (loops pre-expanded)
let cursor   = 0;
let opStartBot;      // bot snapshot at start of current op
let opDuration;      // seconds the current op will take
let opStartedAt;     // millis() when current op began
let lastBot;         // for trail-segment continuity
let bot, trail, ledColor;
let paused = false;
let speedMul = 1.0;
let beeps    = [];   // recent {at, hz, ms} for visual flash
...
function durationFor(op) {
  switch (op.op) {
    ...
    default:        return 0.05;  // pen / led / beep: tiny tick
  }
}

// Interpolate bot pose at progress t∈[0,1] inside the current op.
function applyAt(op, t, start) {
  switch (op.op) {
    ...
    case 'reverse': {
      ...
      // heading 0 = +y (math north); on canvas Y points down, so subtract.
    ...
    case 'arc': {
      // Bot rotates about a center offset perpendicular to current heading.
      const dir = op.right ? 1 : -1;
      const angle = (op.deg | 0) * t * dir;     // deg traversed so far
...
function draw() {
  ...
  // Add trail segment if pen down and we moved
...
function keyPressed() {
  ...
  if (key === 'e' || key === 'E') {
    // Jump to end: render entire program at once
